...
class Entrada2 implements Crud{
	const TABLE = "entrada2";
	...
	public static function insert($data){
		try{
			$connection = Database::instance();
			$sql="INSERT INTO ".self::TABLE."(matricula, base_imponible, tipo_de_gravamen, id_vendedor, id_compraventa, id_comprador, id_tipo, provision, cobrado, id_usuario) VALUES(:matricula, :base_imponible, :tipo_de_gravamen, :id_vendedor, :id_compraventa, :id_comprador, :id_tipo, :provision, :cobrado, :id_usuario)";
			$query = $connection->prepare($sql);
	...
	public static function update($data){
		try{
			$connection = Database::instance();
			$sql = "UPDATE ". self::TABLE. " SET matricula = :matricula, base_imponible = :base_imponible, tipo_de_gravamen = :tipo_de_gravamen, id_vendedor = :id_vendedor, id_compraventa = :id_compraventa, id_comprador = :id_comprador, id_tipo = :id_tipo, id_usuario = :id_usuario where id = :id";
			$query = $connection->prepare($sql);
	...
	public static function updateSalida(){
		try{
			$connection = Database::instance();
			$sql = "UPDATE entrada2 SET entrada2.fecha_salida = (SELECT CURRENT_TIMESTAMP) where entrada2.id = (select enviado.id_entrada from enviado where enviado.id_entrada= entrada2.id)";
			$query = $connection->prepare($sql);
